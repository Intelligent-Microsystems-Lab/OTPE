...
class OSTL(nn.Module):
    ...
    def __call__(self,carry,inputs):
        ...
        def f(snn,carry,inputs):
            if carry['U'].size == 0:
                ...
                #-----------------------------------du_dx-----------------------------------#
                # The "U" in du_dx refers to the membrane potential at the next time-step
                # The "X" in du_dx refers to the membrane potential at the current time-step
                carry['dU_dX'] = jnp.zeros(self.output_sz,self.dtype)
                #############################################################################

                #---------------------------------- a_hat ----------------------------------#
                # a_hat comes from the notation found in the original OTTT paper.
                # https://proceedings.neurips.cc/paper_files/paper/2022/file/82846e19e6d42ebfd4ace4361def29ae-Paper-Conference.pdf
                carry['a_hat'] = jnp.zeros(inputs.size,self.dtype)
                #############################################################################
            carry['U'],s = snn(carry['U'],inputs)
            if len(carry['dU_dParams']) == 0:
                carry['dU_dParams'] = {'params':freeze(tree_map(lambda x: jnp.zeros_like(x,self.dtype),snn.variables)['params'])}
            return carry,s
        ...
        def f_fwd(snn,carry,inputs):
            ...
            J_s_params = tree_map(map_s, carry['dU_dParams'], grad_s_params)
            carry['dU_dParams'] = tree_map(map_u, carry['dU_dParams'], grad_u_params)
    ...
    def initialize_carry(dtype=jnp.float32):
        ...
        carry['dU_dParams'] = jnp.zeros(0,dtype)
        carry['dU_dX'] = jnp.zeros(0,dtype)
        return carry
######################################
